...
void LatteTextureReadbackInfoMtl::StartTransfer()
{
	...
	auto tex = baseTexture->GetTexture();
	if (!tex) return;
	blitCommandEncoder->copyFromTexture(tex, 0, 0, MTL::Origin{0, 0, 0}, MTL::Size{(uint32)baseTexture->width, (uint32)baseTexture->height, 1}, m_mtlr->GetTextureReadbackBuffer(), m_bufferOffset, bytesPerRow, bytesPerImage);
